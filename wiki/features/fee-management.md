...
- [x] ~~Full split of `FeeManagement.tsx`~~ — in progress, done incrementally per #119: Archive, Leavers, Reports, Year-End, and Collect are extracted (`app/school-admin/components/fee-management/`); Overview, Setup (+Applicability), and Student Passbook remain. Parent file 6,478 → 3,778 lines so far.
- [ ] `fmt()`/`fmtDate()` currency/date formatters are copy-pasted into every extracted tab file instead of a shared `fee-management/format.ts` — flagged independently by three review angles. `RemovedStudent` (in `FeeLeaversTab.tsx`) and the local `PendingPayment` redeclaration (in `FeeCollectTab.tsx`) also break the "cross-tab types live in `types.ts`" convention the rest of the split follows. Low risk, mechanical — worth doing once all 8 tabs are extracted and the full duplication surface is known, rather than piecemeal.
