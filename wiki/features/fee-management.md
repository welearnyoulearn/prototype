...
- [x] ~~Full split of `FeeManagement.tsx`~~ — in progress, done incrementally per #119: Archive, Leavers, Reports, Year-End, Collect, Student Passbook, and Overview are extracted (`app/school-admin/components/fee-management/`); only Setup (+Applicability) remains. Parent file 6,478 → 2,784 lines so far. Passbook's payment cancel/correct and waiver revoke/correct stay parent-owned (also used by the Passbook Modal Collect opens directly) and are passed down as `CancelCorrectBundle`/`WaiverCorrectBundle` from `types.ts`.
