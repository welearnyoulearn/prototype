...
| File | Purpose |
|------|---------|
| `app/school-admin/components/FeeManagement.tsx` | Thin tab shell (Overview, Fee Plan, Ledger/Collect, Student Passbook, Reports, Year-End, Past Records, Leavers & Dues) — most tabs now live under `app/school-admin/components/fee-management/` |
...
- [x] ~~Full split of `FeeManagement.tsx`~~ — in progress, done incrementally per #119: Archive, Leavers, Reports, Year-End, Collect, and Student Passbook are extracted (`app/school-admin/components/fee-management/`); only Overview and Setup (+Applicability) remain. Parent file 6,478 → 3,256 lines so far. Passbook's payment cancel/correct and waiver revoke/correct stay parent-owned (also used by the Passbook Modal Collect opens directly) and are passed down as `CancelCorrectBundle`/`WaiverCorrectBundle` from `types.ts`.
- [ ] `fmt()`/`fmtDate()` currency/date formatters are copy-pasted into every extracted tab file instead of a shared `fee-management/format.ts` — flagged independently by three review angles. `RemovedStudent` (in `FeeLeaversTab.tsx`) also breaks the "cross-tab types live in `types.ts`" convention the rest of the split follows (the `PendingPayment`/`CancelCorrectBundle` local redeclarations in `FeeCollectTab.tsx` were fixed during the Passbook slice, since Passbook needed both too). Low risk, mechanical — worth doing once all 8 tabs are extracted and the full duplication surface is known, rather than piecemeal.
