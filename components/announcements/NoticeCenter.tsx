...
const TYPE_EMOJI: Record<string, string> = { general: '📋', circular: '📄', event: '🎉', alert: '🚨' }
...
export default function NoticeCenter({
  ...
  const row = (n: NoticeItem) => {
    ...
    return (
      <button key={n.id} data-testid={`notice-row-${n.id}`} onClick={() => setOpenId(n.id)}
        ...
        <span className="text-2xl leading-none mt-0.5" aria-hidden>{tpl?.emoji ?? TYPE_EMOJI[n.announcement_type] ?? '📋'}</span>
  ...
  return (
    <>
      {items.length > 0 && (
        <div data-testid="notice-center" className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
      ...
      {showAll && (
        <div className="fixed inset-0 z-[250] bg-black/45 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={() => setShowAll(false)}>
          <div data-testid="notice-panel" className="bg-white w-full sm:max-w-xl max-h-[88vh] rounded-t-3xl sm:rounded-3xl shadow-2xl flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="px-5 pt-5 pb-3 space-y-3 border-b border-gray-100">
              <div className="flex items-center justify-between">
                <p className="text-base font-extrabold text-gray-900">📢 {tx.title}</p>
              ...
              <input data-testid="notices-search" value={query} onChange={e => setQuery(e.target.value)} placeholder={tx.search}
