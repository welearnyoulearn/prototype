...
### Added
- Daily Supabase → Cloudflare R2 backup: Vercel Cron (`02:30 UTC`) → `POST /api/cron/backup` writes a gzipped JSON Lines data dump of every public table to R2 (`db/supabase-<ts>.jsonl.gz` + `db/latest.json`), retaining the newest 14 backups. (#NN)
