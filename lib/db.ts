...
export async function initDB() {
  ...
  const migrations = [
    ...
    `DO $$ BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'school_subjects_school_id_grade_subject_name_academic_year_key') THEN
        ALTER TABLE school_subjects ADD CONSTRAINT school_subjects_school_id_grade_subject_name_academic_year_key UNIQUE (school_id, grade, subject_name, academic_year);
      END IF;
    END $$;`
  ]
