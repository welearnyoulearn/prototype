...
export default function StudentSetGenderPage() {
  ...
  useEffect(() => {
    fetch('/api/student/auth/me')
      .then(async r => {
        ...
        if (data.gender) { router.push('/student/set-avatar'); return }
        setChecking(false)
      })
  ...
  async function choose(gender: 'male' | 'female' | null) {
    setSaving(true)
    try {
      await fetch('/api/student/auth/gender', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ gender }),
      })
    } finally {
      router.push('/student/set-avatar')
    }
  }
