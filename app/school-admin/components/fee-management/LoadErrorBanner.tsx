...
export function LoadErrorBanner({ message, onRetry }: { message?: string; onRetry: () => void }) {
  ...
  return (
    <div className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-xl px-4 py-3 text-sm text-red-700">
      ...
      <button data-testid="btn-load-error-retry" onClick={onRetry} className="text-xs font-semibold text-red-700 underline underline-offset-2 hover:text-red-900">Retry</button>
    </div>
  )
}
