...
// Every field pulled from `rep` below is stored text (names, reasons, notes,
// receipt numbers, category labels…) that this same admin UI lets other
// admins/staff write elsewhere (waiver reasons, student names, etc.) — it is
// NOT guaranteed safe to drop into `document.write`'s raw HTML unescaped.
// Every interpolation of such a field goes through this. Numbers already run
// through RUPEE() and dates through `new Date(...).toLocaleString()` are
// left bare — those are computed, not user-authored text.
...
function buildAuditPdfHtml(rep: Record<string, unknown>): string {
  ...
  const head = `
    <div class="hdr">
      ...
      <div class="sub">${esc(meta.school_name)} · ${esc(meta.academic_year)}${meta.scope ? ' · ' + esc(meta.scope) : ''}</div>
      <div class="meta">Generated by ${esc(meta.generated_by)} on ${new Date(meta.generated_on).toLocaleString('en-IN')}</div>
    </div>`
  const style = `<style>
  ...
  if (rep.kind === 'student') {
    ...
    return `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Student Fee Report</title>${style}</head><body>${head}
      ...
      <tbody>${pays.map(p => `<tr><td>${esc(p.receipt_number)}</td><td>${esc(p.paid_date)}</td><td>${esc(p.fee_type)} · ${esc(p.period_label)}</td><td class="r">${RUPEE(p.amount)}</td><td>${esc(p.payment_mode)}</td><td>${esc(p.payment_status)}</td></tr>`).join('') || '<tr><td colspan="6">No payments</td></tr>'}</tbody></table>
      <h3>Waivers</h3>
      ...
      <tbody>${wvs.map(w => `<tr><td>${esc(w.fee_type)} · ${esc(w.period_label)}</td><td class="r">${RUPEE(w.waiver_amount)}</td><td>${esc(w.reason)}</td><td>${esc(w.granted_by_name) || ''}</td><td>${w.is_revoked ? 'Revoked' : 'Active'}</td><td>${w.is_revoked ? `${esc(w.revoked_by) || ''} · ${w.revoked_at ? new Date(w.revoked_at).toLocaleString('en-IN') : ''} · ${esc(w.revoke_reason) || ''}` : ''}</td></tr>`).join('') || '<tr><td colspan="6">No waivers</td></tr>'}</tbody></table>
      </body></html>`
  }
  ...
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Fee Audit Report</title>${style}</head><body>${head}
    ...
    <tbody>${byType.map(t => `<tr><td>${esc(t.fee_type)}</td>${mrow(t)}</tr>`).join('')}${totRow('GRAND TOTAL', sm, 1)}</tbody></table>
    <h3>4. Fee Change Log</h3>
    <table><thead><tr><th>Date & Time</th><th>Action</th><th>Detail</th><th class="r">Amount</th><th>User</th></tr></thead>
    <tbody>${log.map(l => `<tr><td>${new Date(l.at).toLocaleString('en-IN')}</td><td>${esc(l.type)}</td><td>${esc(l.detail)}</td><td class="r">${l.amount != null ? RUPEE(l.amount) : ''}</td><td>${esc(l.user) || ''}</td></tr>`).join('') || '<tr><td colspan="5">No changes recorded</td></tr>'}</tbody></table>
    ...
    <tbody>${byClass.map(c => `<tr><td>${esc(c.class)}</td><td>${esc(c.fee_type)}</td>${mrow(c)}</tr>`).join('')}${totRow('GRAND TOTAL', sm, 2)}</tbody></table>
    ...
    <tbody>${byStudent.map(s => s.is_subtotal
      ? `<tr class="tot"><td colspan="3">${esc(s.student)}</td><td>SUBTOTAL</td>${mrow(s)}</tr>`
      : `<tr><td>${esc(s.student)}</td><td>${esc(s.roll_number)}</td><td>${esc(s.class)}</td><td>${esc(s.fee_type)}</td>${mrow(s)}</tr>`).join('')}${totRow('GRAND TOTAL', sm, 4)}</tbody></table>
    </body></html>`
}
