...
export default function FeeManagement({
  ...
  // ── Past Records (Archive): every academic year's headline + close status ──
  type ArchiveYear = {
    academic_year: string
    start_date: string
    end_date: string
  ...
  const loadArchive = useCallback(async () => {
    ...
    try {
      ...
      if (r.ok) { clearLoadError('archive'); const d = await r.json(); setArchiveYears(d.years || []) }
      else setLoadError('archive', 'Could not load past records — try refreshing')
    } catch { setLoadError('archive', 'Network error — past records could not be loaded') }
    finally { setArchiveLoading(false) }
  }, [schoolId])
  useEffect(() => { if (activeTab === 'archive') loadArchive() }, [activeTab, loadArchive])
  ...
  function viewArchiveYear(year: string, dest: 'reports' | 'collect') {
    ...
    if (dest === 'collect') setCollectionView('counter')
  }
...
<div class="ftr">Generated ${new Date().toLocaleString('en-IN')} · Computer-generated statement.</div>
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'archive' && (
        <div className="space-y-5">
          <LoadErrorBanner sectionKey="archive" onRetry={loadArchive} />
          <div className="flex items-center justify-between">
            <div>
              ...
              {archiveYears.map(y => {
                ...
                return (
                  <div key={y.academic_year} className="bg-white rounded-xl border border-gray-100 p-5">
                    ...
                    <div className="flex items-center gap-2 mt-4 pt-3 border-t border-gray-50">
                      <button data-testid={`btn-archive-report-${y.academic_year}`} onClick={() => viewArchiveYear(y.academic_year, 'reports')}
                        className="text-xs bg-blue-600 text-white px-3 py-1.5 rounded-lg font-medium hover:bg-blue-700">
                        View Report →
                      </button>
                      <button data-testid={`btn-archive-ledger-${y.academic_year}`} onClick={() => viewArchiveYear(y.academic_year, 'collect')}
