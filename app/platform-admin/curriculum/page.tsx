...
export default function PlatformCurriculum() {
  ...
  const handleUploadFileToR2 = async (file: File, onError?: (msg: string) => void, materialType?: string) => {
    if (!file) return null
    setUploading(true)
    setUploadProgress(0)
    setUploadError('')
    try {
      const signRes = await fetch('/api/platform/materials/upload-sign', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ filename: file.name, content_type: file.type || 'application/pdf', subject_id: activeSubject?.id, material_type: materialType }),
      })
      const signData = await signRes.json()
  ...
  const handleUploadMaterial = async (e: React.FormEvent) => {
    ...
    for (const file of newMaterialFiles) {
      ...
      try {
        let uploadErrMsg = ''
        const url = await handleUploadFileToR2(file, msg => { uploadErrMsg = msg }, materialTypeToUpload ?? undefined)
    ...
    for (const group of importGroups) {
      ...
      for (const book of group.books) {
        try {
          ...
          // a handbook until that table's CHECK is widened. The R2 key still
          // uses the real classification (workbooks get their own folder)
          // since that's independent of the DB constraint.
          ...
          let uploadErrMsg = ''
          const url = await handleUploadFileToR2(book.file, msg => { uploadErrMsg = msg }, classification)
