...
function ResetForm() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/teacher/auth/reset-password?token=${token}`)
      .then(r => r.json())
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/teacher/auth/reset-password', {
        method: 'POST',
      ...
      setDone(true)
      setTimeout(() => router.push('/teacher/login'), 2500)
    } catch {
  ...
  if (valid === null) return <p className="text-center text-gray-500 py-8">Validating link...</p>
  ...
  if (!valid) return (
    <div className="text-center py-4">
      ...
      <p className="text-sm text-gray-500 mb-5">This reset link is no longer valid. Please request a new one.</p>
      <Link href="/teacher/forgot-password" className="text-emerald-600 font-medium text-sm hover:text-emerald-800 transition">Request new link →</Link>
    </div>
  )
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <PasswordField label="Confirm New Password" value={confirm} onChange={setConfirm} placeholder="Repeat your new password" ring={theme.ring} autoComplete="new-password" />
...
export default function TeacherResetPasswordPage() {
  return (
    <AuthShell theme={THEMES.teacher} title="Reset Your Password" subtitle="Enter a new password for your account">
