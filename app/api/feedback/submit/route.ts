...
// Body: { code, role, is_anonymous, name?, phone?, ratings:[{category_key,rating}], quick_picks?, free_text?, voice_key? }
//
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const duplicateKeys = body.ratings.map(r => r.category_key).filter((k, i, arr) => arr.indexOf(k) !== i)
    ...
    // category_key that doesn't belong to this school.
    const categoryKeys = body.ratings.map(r => r.category_key)
    ...
    const { rows: [submission] } = await client.query(
      `INSERT INTO feedback_submissions
         (school_id, role, is_anonymous, submitter_name, submitter_phone, quick_pick_tags, free_text, voice_object_key, ip_hash)
    ...
    // Single multi-row insert rather than one round-trip per rating — this
    // is a public, unauthenticated hot path (every QR scan hits it).
    const ratingParams: unknown[] = []
    const ratingRows = body.ratings.map(r => {
