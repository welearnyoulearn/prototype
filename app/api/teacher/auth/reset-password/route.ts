  ...
  try {
    ...
    const result = await pool.query(
      `SELECT * FROM password_reset_tokens
       WHERE token = $1 AND role = 'teacher' AND used = FALSE AND expires_at > NOW()`,
      [token]
    )
    ...
    await pool.query('UPDATE teachers SET password_hash = $1, password_changed = TRUE WHERE id = $2', [newHash, rec.reference_id])
